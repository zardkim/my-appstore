# MyApp Store 테스트 방법

이 문서는 MyApp Store를 다양한 환경에서 테스트하는 방법을 설명합니다.

## 목차
1. [Synology NAS 환경 테스트](#synology-nas-환경-테스트)
2. [로컬 Docker Compose 테스트](#로컬-docker-compose-테스트)
3. [로컬 개발 환경 테스트](#로컬-개발-환경-테스트)
4. [API 테스트](#api-테스트)
5. [기능별 테스트 시나리오](#기능별-테스트-시나리오)

---

## Synology NAS 환경 테스트

### 1. 설치 및 실행

자세한 설치 방법은 [Synology 설치 가이드](./SYNOLOGY_INSTALLATION.md)를 참조하세요.

```bash
# SSH로 NAS 접속
ssh admin@YOUR_NAS_IP

# 프로젝트 디렉토리로 이동
cd /volume1/docker/myappstore

# 서비스 시작
docker-compose up -d

# 로그 확인
docker-compose logs -f
```

### 2. 접속 확인

**프론트엔드:**
```
http://YOUR_NAS_IP:5900
```

**백엔드 API:**
```
http://YOUR_NAS_IP:8100/api/health
http://YOUR_NAS_IP:8100/docs  (Swagger UI)
```

### 3. 기본 테스트 시나리오

#### 테스트 1: 초기 설정
1. 브라우저에서 `http://YOUR_NAS_IP:5900` 접속
2. 자동으로 `/setup` 페이지로 리다이렉트 확인
3. 관리자 계정 생성
4. 로그인 성공 확인

#### 테스트 2: 스캔 기능
1. 테스트 파일 생성:
   ```bash
   # SSH에서
   mkdir -p /volume1/docker/myappstore/data/library/TestApp
   touch "/volume1/docker/myappstore/data/library/TestApp/Adobe Photoshop 2024 v25.0.exe"
   ```

2. 웹 UI에서:
   - `관리자` → `스캔` 탭
   - 스캔 경로: `/library`
   - `스캔 시작` 클릭
   - 결과 확인

#### 테스트 3: 제품 조회
1. `스토어` 메뉴 클릭
2. 스캔된 제품 목록 확인
3. 제품 상세 페이지 확인
4. 버전 정보 확인

#### 테스트 4: 패치 링크 기능 (v1.2.2-beta)
1. 제품 상세 페이지 → `패치/크랙` 탭
2. 관리자: 링크 추가 버튼 확인
3. 링크 추가 테스트:
   - 제목: "공식 다운로드"
   - URL: "https://example.com"
   - 저장 후 표시 확인

### 4. 로그 확인

```bash
# 전체 로그
docker-compose logs

# 실시간 로그 (특정 서비스)
docker-compose logs -f backend
docker-compose logs -f frontend

# 에러만 필터링
docker-compose logs | grep -i error
```

### 5. 성능 테스트

```bash
# 컨테이너 리소스 사용량
docker stats

# 데이터베이스 연결 테스트
docker exec -it myapp-db psql -U postgres -d myappstore -c "SELECT COUNT(*) FROM products;"

# Redis 연결 테스트
docker exec -it myapp-redis redis-cli ping
```

---

## 로컬 Docker Compose 테스트

### 방법 1: Docker Compose 사용 (권장)

전체 스택(DB + Backend + Frontend)을 한 번에 실행:

```bash
# 프로젝트 루트에서
cd /home/nuricom/project/myappStore

# Docker Compose로 모든 서비스 시작
docker-compose up -d

# 로그 확인
docker-compose logs -f backend

# 중지
docker-compose down
```

**접속 주소:**
- 백엔드: http://localhost:8100
- API 문서: http://localhost:8100/docs (Swagger UI)
- 프론트엔드: http://localhost:5900

---

## 로컬 개발 환경 테스트

### 방법 2: 로컬 직접 실행 (개발용)

더 빠른 개발/테스트를 위한 방법:

#### 1. 데이터베이스만 Docker로 실행

```bash
cd /home/nuricom/project/myappStore
docker-compose up -d db redis
```

#### 2. 백엔드 실행

```bash
# 백엔드 디렉토리로 이동
cd backend

# 가상환경 활성화 (이미 venv가 있으면)
source venv/bin/activate

# 의존성 설치 (처음 한 번만)
pip install -r requirements.txt

# 데이터베이스 마이그레이션
alembic upgrade head

# 서버 실행 (개발 모드)
uvicorn app.main:app --host 0.0.0.0 --port 8110 --reload
```

**옵션 설명:**
- `--host 0.0.0.0`: 모든 네트워크 인터페이스에서 수신
- `--port 8100`: 포트 8100에서 실행
- `--reload`: 코드 변경 시 자동 재시작 (개발용)

#### 3. 프론트엔드 실행

```bash
# 새 터미널에서
cd frontend

# 의존성 설치 (처음 한 번만)
npm install

# 개발 서버 실행
npm run dev
```

### 환경 변수 확인

`backend/.env` 파일 확인:
```bash
DATABASE_URL=postgresql://postgres:password@localhost:5432/myappstore
REDIS_URL=redis://localhost:6379/0
SECRET_KEY=your-secret-key
OPENAI_API_KEY=sk-your-api-key  # 선택사항
```

---

## API 테스트

### Swagger UI 사용

브라우저에서 `http://localhost:8100/docs` 접속:

1. **인증 테스트:**
   - `/api/auth/login` 엔드포인트 선택
   - `Try it out` 클릭
   - 사용자명과 비밀번호 입력
   - `Execute` 클릭
   - 응답에서 `access_token` 확인

2. **인증 토큰 설정:**
   - 우측 상단 `Authorize` 버튼 클릭
   - `Bearer YOUR_TOKEN` 형식으로 입력
   - `Authorize` 클릭

3. **API 호출:**
   - 원하는 엔드포인트 선택
   - `Try it out` 클릭
   - 필요한 파라미터 입력
   - `Execute` 클릭

### curl 사용

```bash
# Health Check
curl http://localhost:8100/api/health

# 로그인
curl -X POST "http://localhost:8100/api/auth/login" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "username=admin&password=yourpassword"

# 제품 목록 조회 (인증 필요)
TOKEN="your_access_token"
curl -X GET "http://localhost:8100/api/products/" \
  -H "Authorization: Bearer $TOKEN"

# 통계 조회
curl -X GET "http://localhost:8100/api/products/stats/overview" \
  -H "Authorization: Bearer $TOKEN"
```

### Postman/Insomnia 사용

1. 새 컬렉션 생성
2. Authorization 설정:
   - Type: Bearer Token
   - Token: 로그인으로 받은 access_token
3. 요청 생성 및 테스트

---

## 기능별 테스트 시나리오

### 1. 인증 및 권한

#### 초기 설정
```bash
# 1. 설정 페이지 확인
curl http://localhost:8100/api/auth/check-setup
# 응답: {"setup_required": true}

# 2. 관리자 계정 생성
curl -X POST "http://localhost:8100/api/auth/setup" \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"secure123"}'
```

#### 로그인
```bash
curl -X POST "http://localhost:8100/api/auth/login" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "username=admin&password=secure123"
```

### 2. 파일 스캔

#### 수동 스캔
```bash
# 관리자 토큰으로
curl -X POST "http://localhost:8100/api/scan/start" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "scan_path": "/library",
    "use_ai": false
  }'
```

#### 스케줄러 설정
```bash
# 스케줄러 시작
curl -X POST "http://localhost:8100/api/scheduler/start" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "cron_schedule": "0 2 * * *",
    "scan_paths": ["/library"],
    "use_ai": true
  }'

# 스케줄러 상태 확인
curl -X GET "http://localhost:8100/api/scheduler/status" \
  -H "Authorization: Bearer $ADMIN_TOKEN"
```

### 3. 제품 관리

#### 제품 목록 조회
```bash
# 기본 목록
curl "http://localhost:8100/api/products/" \
  -H "Authorization: Bearer $TOKEN"

# 필터링
curl "http://localhost:8100/api/products/?category=Graphics&vendor=Adobe" \
  -H "Authorization: Bearer $TOKEN"

# 검색
curl "http://localhost:8100/api/products/?search=photoshop" \
  -H "Authorization: Bearer $TOKEN"
```

#### 제품 상세 조회
```bash
curl "http://localhost:8100/api/products/1" \
  -H "Authorization: Bearer $TOKEN"
```

#### 제품 메타데이터 수정 (관리자)
```bash
curl -X PUT "http://localhost:8100/api/products/1" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "Adobe Photoshop 2024",
    "description": "Professional image editing software",
    "category": "Graphics"
  }'
```

### 4. 패치 링크 기능 (v1.2.2)

#### 링크 추가
```bash
curl -X PUT "http://localhost:8100/api/products/1" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "patch_links": [
      {
        "title": "공식 다운로드",
        "url": "https://example.com/download"
      },
      {
        "title": "패치 파일",
        "url": "https://example.com/patch"
      }
    ]
  }'
```

#### 링크 조회
```bash
curl "http://localhost:8100/api/products/1" \
  -H "Authorization: Bearer $TOKEN" | jq '.patch_links'
```

### 5. 첨부 파일 (패치/크랙)

#### 파일 업로드
```bash
curl -X POST "http://localhost:8100/api/attachments/upload" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -F "file=@/path/to/patch.zip" \
  -F "product_id=1" \
  -F "type=patch" \
  -F "note=Version 2.0 patch"
```

#### 파일 목록 조회
```bash
curl "http://localhost:8100/api/attachments/product/1" \
  -H "Authorization: Bearer $TOKEN"
```

### 6. 즐겨찾기 및 스크랩

#### 즐겨찾기 추가
```bash
curl -X POST "http://localhost:8100/api/favorites/1" \
  -H "Authorization: Bearer $TOKEN"
```

#### 스크랩 추가
```bash
curl -X POST "http://localhost:8100/api/scraps/1" \
  -H "Authorization: Bearer $TOKEN"
```

### 7. 설정 관리

#### 카테고리 조회
```bash
curl "http://localhost:8100/api/config/categories" \
  -H "Authorization: Bearer $TOKEN"
```

#### 카테고리 업데이트 (관리자)
```bash
curl -X PUT "http://localhost:8100/api/config/categories" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" \
  -d '[
    {"name": "Graphics", "label": "그래픽", "icon": "🎨"},
    {"name": "Office", "label": "오피스", "icon": "📊"}
  ]'
```

---

## 포트 충돌 해결

### 포트 사용 확인

```bash
# Linux/Mac
lsof -ti:8100 | xargs kill -9  # 포트 8100 강제 종료
sudo lsof -t -i:8100           # 포트 8100 사용 프로세스 확인
netstat -nap | grep 8100       # 포트 8100 상태 확인

# Synology
netstat -tuln | grep 8100
```

### Docker Compose 포트 변경

`docker-compose.yml` 수정:
```yaml
services:
  backend:
    ports:
      - "8101:8100"  # 8100 대신 8101 사용
  frontend:
    ports:
      - "5901:5900"  # 5900 대신 5901 사용
```

---

## 데이터베이스 관리

### PostgreSQL 접속

```bash
# Docker 환경
docker exec -it myapp-db psql -U postgres -d myappstore

# 로컬 환경
psql -U postgres -d myappstore
```

### 유용한 SQL 쿼리

```sql
-- 제품 수 확인
SELECT COUNT(*) FROM products;

-- 카테고리별 제품 수
SELECT category, COUNT(*)
FROM products
GROUP BY category
ORDER BY COUNT(*) DESC;

-- 최근 스캔 히스토리
SELECT * FROM scan_history
ORDER BY started_at DESC
LIMIT 5;

-- 사용자 목록
SELECT id, username, role, created_at FROM users;
```

### 데이터베이스 백업 및 복원

```bash
# 백업
docker exec myapp-db pg_dump -U postgres myappstore > backup_$(date +%Y%m%d).sql

# 복원
docker exec -i myapp-db psql -U postgres myappstore < backup_20260119.sql
```

---

## 로그 분석

### 백엔드 로그

```bash
# Docker
docker-compose logs -f backend

# 에러만 확인
docker-compose logs backend | grep -i error

# 특정 시간 이후 로그
docker-compose logs --since "2026-01-19T10:00:00" backend
```

### 프론트엔드 로그

```bash
# Docker
docker-compose logs -f frontend

# 브라우저 콘솔
# F12 → Console 탭
```

### 데이터베이스 로그

```bash
# PostgreSQL 로그 (Synology)
tail -f /volume1/docker/myappstore/data/logs/postgresql/*.log

# Docker
docker-compose logs db
```

---

## 성능 테스트

### 응답 시간 측정

```bash
# curl로 응답 시간 측정
curl -o /dev/null -s -w "Time: %{time_total}s\n" \
  http://localhost:8100/api/products/

# Apache Bench
ab -n 100 -c 10 http://localhost:8100/api/products/
```

### 부하 테스트

```bash
# wrk 사용
wrk -t4 -c100 -d30s http://localhost:8100/api/products/

# 4 threads, 100 connections, 30 seconds
```

---

## 문제 해결

### 백엔드 시작 실패

```bash
# 로그 확인
docker-compose logs backend

# 데이터베이스 연결 확인
docker exec myapp-backend ping -c 3 db

# 마이그레이션 상태 확인
docker exec myapp-backend alembic current
```

### 프론트엔드 빌드 오류

```bash
# node_modules 재설치
rm -rf frontend/node_modules
docker-compose build frontend --no-cache
docker-compose up -d frontend
```

### 데이터베이스 연결 오류

```bash
# PostgreSQL 재시작
docker-compose restart db

# 연결 테스트
docker exec myapp-db pg_isready -U postgres
```

---

## 추가 리소스

- **Swagger UI**: http://localhost:8100/docs
- **ReDoc**: http://localhost:8100/redoc
- **프로젝트 GitHub**: https://github.com/zardkim/my-appstore
- **Synology 설치 가이드**: [SYNOLOGY_INSTALLATION.md](./SYNOLOGY_INSTALLATION.md)
- **CLAUDE.md**: 프로젝트 아키텍처 가이드
