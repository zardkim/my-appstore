                ...
                <tr v-for="user in users" :key="user.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td class="px-6 py-4 text-right">
                    <div class="flex items-center justify-end space-x-2">
                      <button v-if="user.role !== 'admin'" @click="resetUserPassword(user)" class="text-purple-600 dark:text-purple-400 hover:text-purple-800 dark:hover:text-purple-300 font-medium text-sm">{{ t('settings.users.resetPassword') }}</button>
          ...
          <div v-if="isAdmin" class="md:hidden space-y-3">
            ...
            >
              ...
              <div v-if="user.role !== 'admin'" class="flex gap-2">
                <button @click="resetUserPassword(user)" class="flex-1 px-3 py-2 text-sm text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20 rounded-lg hover:bg-purple-100 dark:hover:bg-purple-900/30 font-medium transition-colors">{{ t('settings.users.resetPassword') }}</button>
...
const resetUserPassword = async (user) => {
  const shouldReset = await confirm.danger(
    t('settings.users.resetPasswordConfirm', { username: user.username }),
  ...
  try {
    await usersApi.changePassword(user.id, randomPassword)
    await alert.success(t('settings.users.resetPasswordSuccess', { password: randomPassword }))
  } catch (error) {
    console.error('Password reset error:', error)
    await alert.error(error.response?.data?.detail || t('settings.users.resetPasswordFailed'))
  }
}
